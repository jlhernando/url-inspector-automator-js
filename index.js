...
const { readFileSync, writeFileSync } = require('fs') // Module to access the File System - Extract only ones needed
...
/* Data */
const rawFile = readFileSync(file, 'utf8') // Read urls.csv file
...
;(async () => {
  ...
  for (const [index, url] of urls.entries()) {
    ...
    if (await page.$('text=Quota exceeded')) {
      ...
      const coverage = await page.evaluate(
        () => document.querySelector('.mzcHEd').textContent
      )
      // Add to obj
      obj.coverage = coverage

      // Extract top level info and push to obj
      const indexState = await page.$eval('.CC5fre', (el) => el.textContent)
      console.log(indexState)

      // Extract top level info description
      const indexStateDesc = await page.evaluate(
        () => document.querySelector('.kXvEDd').textContent
      )
      // Add to obj
      obj['index state description'] = indexStateDesc

      // Extract accordion values
      await page.click('.QB1Nub')
      const lastCrawl = await page.evaluate(
        () => document.querySelector('.zVF5Ie').textContent
      )
      // Add to obj
      obj['last crawl'] = lastCrawl
      ...
      console.log(finalObj) // Log it to the console
  ...
  // Parse JSON to CSV
  const parseCsv = new Parser() // Create CSV parser
  const csv = parseCsv.parse(results) // Parse it
  writeFileSync('./results.csv', csv) // Write file
})()
